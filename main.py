    ...
    r=requests.request(
        ...
        data=json.dumps({
            ...
            "messages": [
                ...
                {"role": "user", "content": f"请写一篇主题为{subject}的作文，字数在{n_words}左右,额外要求:{other_needs}"},
            ]
        }),
    )
...
def main():
    ...
    llm_model=LLM_MODELS[int(input("选择模型(1.deepseek-r1 2.qwen-plus 3.qwen-turbo)"))]
    while True:
        try:
            article_type=ARTICLE_TYPE[int(input("请输入文体(1.记叙文 2.议论文 3.说明文): "))]
            break
        except:
            print("输入1或2或3")
    subject=input("请输入主题: ")
    ...
    print("AI生成中...")
    try:
        print(do_request(api_key,subject,n_words,article_type,other_needs,llm_model)['choices'][0]['message']['content'])
    except:
